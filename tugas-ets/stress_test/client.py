...
class Client:
    ...
    def send_command(self, command_str):
        ...
        try:
            ...
            # Send data in chunks to avoid memory issues with large files
    ...
    def record_upload(self, file_path, worker_id):
        start_time = time.time()
        filename = os.path.basename(f'worker_{worker_id}_{file_path}')
        file_size = os.path.getsize(file_path)

        try:
            logging.info(f"Worker {worker_id} - PUT command for {filename} ({file_size/1024/1024:.2f} MB)")
            ...
            if result['status'] == 'OK':
                logging.info(f"Worker {worker_id} - PUT command successful, uploaded {filename} ({file_size/1024/1024:.2f} MB) in {duration:.2f}s - {throughput/1024/1024:.2f} MB/s")
                self.success_count['upload'] += 1
            else: 
                logging.error(f"Worker {worker_id} - PUT command failed: {result['data']}")
            ...
            end_time = time.time()
            duration = end_time - start_time
            logging.error(f"Worker {worker_id} - PUT command error: {str(e)}")
            self.fail_count['upload'] += 1
    ...
    def run_test(self, operation, file_size_mb, client_pool_size, executor_type='thread', batch_size=None):
        """
        Run a stress test with the given parameters.
        
        Args:
            operation: 'upload', 'get', or 'list'
            file_size_mb: File size in megabytes
            client_pool_size: Number of concurrent clients
            executor_type: 'thread' or 'process'
            batch_size: Number of workers to run at once (None = all)
        """
        self.reset_counters()
        ...
        # Upload original file to files if needed
        if operation == 'get':
        ...
        # Use batching if specified to avoid resource exhaustion
        if batch_size is None or batch_size >= client_pool_size:
            batch_size = client_pool_size
            num_batches = 1
        else:
            num_batches = (client_pool_size + batch_size - 1) // batch_size  # ceiling division
        
        for batch in range(num_batches):
            start_idx = batch * batch_size
            end_idx = min((batch + 1) * batch_size, client_pool_size)
            current_batch_size = end_idx - start_idx
            
            logging.info(f"Running batch {batch+1}/{num_batches} with {current_batch_size} workers")
            
            with executor_class(max_workers=current_batch_size) as executor:
                futures = []
    ...
    def cleanup(self):
        ...
        # Only remove worker-generated files from 'files' directory
    ...
    def perform_stress_test(self, operations, file_sizes, client_pool_sizes, executor_types=['thread'], batch_size=None):
        all_stats = []
    ...
    def automate_stress_test(self, batch_size=None):
        """
        Run automated stress tests with predefined parameters.
        
        Args:
            batch_size: If provided, limits the number of concurrent workers
                       to avoid resource exhaustion.
        """
        operations = ['get', 'upload', 'list']
        file_sizes = [10, 100]
        client_pool_sizes = [1, 5, 10, 20]  # Keep this reasonable or use batching
        executor_types = ['thread', 'process']
        
        # For higher client counts, enforce batching
        if batch_size is None:
            # Default batch size if not specified
            batch_size = 10
            
        csv_file = self.perform_stress_test(
            operations, file_sizes, client_pool_sizes, executor_types, batch_size
        )
